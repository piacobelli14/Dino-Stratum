...
The summary tab is the AI-generated briefing for the selected event. It starts by gathering a minimal context bundle from the available upstream sources (news articles, social posts, academic papers, related risks in the area) and passes the whole package along with the risk's metadata, location, population impact, and any nearby org assets to a Gemini 2.0 pipeline that returns an executive summary, a list of key findings, a statistics table, an impact assessment with affected areas, a recommendations list, an event timeline, and a self-reported confidence level with rationale. The whole thing is rendered as a grid of cards: main summary on the left, then a confidence card with a tiered HIGH/MEDIUM/LOW indicator and a breakdown of how many tier-one outlets, tier-two outlets, distinct sources, and academic references corroborate the event, then a source bundle viewer, then stats, impact, recommendations, and timeline. If Gemini is unavailable for any reason, the bar falls back to a deterministic summary built from the raw risk metadata and source counts so the tab is never empty.

The bar supports two viewing scopes that can be toggled per user. In viewport scope, the briefing is written as a general operational summary of the event, oriented toward whoever is looking at it. In assets scope, the same briefing is generated but the impact assessment, recommendations, and key findings are biased toward the org's assets that fall within the event's impact radius. The asset list is pulled from the PostGIS asset registry at briefing time, sorted by criticality and distance, and fed into the prompt so the resulting summary calls out specific named assets that may be affected.

The details tab is the structured event dump. It is rendered as a horizontally scrolling row of cards that vary based on the risk category. An overview card with the title, severity badge, score, and quick actions (source link, location assessment, nearby query) is always present. A location and timing card shows the event time, last update, expiry, coordinates, impact radius, and geometry type. A population impact card surfaces density tier, estimated population, nearest city, and distance to it when the risk has a resolved location. The detailed data card flattens the entire metadata blob into a clean grid - magnitude and depth for seismic events, percent contained and acres for wildfires, NWS severity and urgency for weather alerts, displacement and coherence for ground deformation, alert level and population affected for GDACS events, and so on for every category. There are also recommendations and source info cards. When a body is selected through the Overpass identify path instead of a risk, this tab swaps to show the feature's measurements (area, perimeter, vertex count, centroid), address, elevation, and zoom-to-feature controls.

The articles tab pulls news coverage of the event from GDELT and GNews, deduplicated by URL and scored against the event by a combination of source tier, recency relative to the event time, and keyword overlap with the event's title and category. Every article is then passed through a Gemini relevance filter that scores each one zero through ten against the actual event (so a generic flood preparedness article scores low while one about this specific flood scores high) and items below threshold are dropped. Articles surface as cards with thumbnails, the source name, relative publish time, a relevance badge, and the provider tag. If Gemini is unavailable, a keyword-based fallback filter runs instead.

The videos tab does the same thing for YouTube. It pulls candidates through the YouTube Data API filtered by event time window, scores them by view count, channel verification status, and keyword overlap, then runs them through the same Gemini relevance pipeline. Each card has the thumbnail, channel title, view count, duration, and a verified/likely/unverified badge driven by the AI relevance score.

The images tab merges results from Google Custom Search image queries, Wikimedia Commons (which is preferred for licensing reasons), and images pulled directly from the news articles in the articles tab. The whole pool is deduplicated by URL and run through a stricter Gemini visual relevance filter that drops generic stock images, logos, and maps of the wrong area. The grid is clickable, opening a full-screen viewer with previous and next controls and a caption pointing back to the source.

The academic tab queries the Semantic Scholar API for peer-reviewed research relevant to the event's category and location. The query is built from a hand-curated category-to-term map (seismic events query "seismology earthquake engineering," wildfires query "wildfire fire science combustion," and so on for every category) plus the event's location string. Results show the paper title, author list, abstract excerpt, publication year, citation count, and a direct link to the DOI or paper URL.

The social tab queries Reddit's public search with the event's location and category and returns recent posts sorted by relevance. The Gemini relevance filter is particularly aggressive here because Reddit results are noisy. Each card shows the subreddit, the author, score, comment count, time posted, and a relevance badge that surfaces whether the post is actually about the event or just generic discussion.
...
Every briefing the intel bar generates is persisted server-side with its source bundle, confidence metrics, scope, asset count, and viewport bounding box. This means every claim in every summary has its corroborating sources recorded, so users can flag a briefing through the feedback button in the summary header and the recorded sources are captured alongside the flag. The feedback modal collects a category (inaccurate summary, wrong location, off-topic sources, hallucinated content, outdated information, other), a reason, and free-form details. Submitted feedback is searchable for review.

There is also a per-user source toggle settings panel in the summary header. Each upstream provider (GDELT, GNews, Google CSE, YouTube, Google Images, Wikimedia, Semantic Scholar, Reddit, ReliefWeb, Wikipedia) can be turned on or off, and the user's default viewing scope can be set to viewport or assets. Settings are stored server-side keyed by username and org, and disabled sources are skipped entirely on all subsequent briefing fetches. This serves both as a user preference layer and a way to debug bad summaries by isolating which provider is contributing noise.

In-flight briefing requests are deduplicated by risk ID and scope, so rapid-fire clicks across the same risk don't trigger duplicate Gemini calls or duplicate upstream fetches. The same dedup pattern applies to the lighter summary-only endpoint, which is used when the user just wants the AI summary without the full media gallery.

#### Risk Layer Toggles

The platform monitors eight hazard families simultaneously: earthquakes, wildfires, weather alerts, floods, volcanoes, air quality, ground deformation, and global disasters. Each family can be toggled independently from the sidebar or from a dedicated risk layers modal that shows the live event count per family with category icons. Toggling a family off hides its markers, polygons, and impact circles from the map and removes its contributions from the sidebar summary counts. The modal also has show-all and hide-all shortcuts. The list of categories is intentionally fixed because each family has its own normalizer on the backend that knows how to extract location, severity, and metadata from that family's upstream feed.

#### My Area

Most users care about a specific region, not the whole planet. The My Area feature lets you define a personal area of interest as either a point with a radius (circular, useful for "alerts within 100 km of my office") or a bounding box (rectangular, useful for "alerts within this state"). The point mode supports forward geocoding through Nominatim, so you can type an address and the platform resolves it to coordinates. There is also a use-current-map-center shortcut for the point mode and a use-current-map-view shortcut for the bounding box mode. The saved area is mirrored to both PostGIS and local storage so it loads instantly on next visit. When the area filter is active, every alert, summary count, sidebar list, and exposure number in the page filters down to events inside the area only. The map also renders a dashed boundary overlay and a labeled center marker so it is always visually clear what region you are scoped to.

#### Saved Views

The saved views feature is the bookmark layer. At any moment you can capture the current camera position (center, zoom, pitch, bearing, computed bounding box), all of the active filters (risk layer toggles, my area state), and the active map layer state (satellite vs topo, asset visibility mode, heatmap, visibility badges) into a named view with an optional description. Saved views are stored both client-side in local storage and server-side keyed by username and org, with conflict-free merging on load. Each saved view can be applied with a single click, which flies the map back to the captured position and restores the filter and layer state. Up to fifty views per user are kept.

#### Location Assessment

The location assessment feature runs a comprehensive risk evaluation against any point on the map. From the header button or the map controls, you can trigger an assessment against the current map center with a configurable radius. The backend pulls all risk events from all categories within the radius, computes a weighted risk score driven by each event's severity, distance, and contribution to the overall composite, and returns a structured assessment with the overall score, risk level (Critical/High/Medium/Low), the top contributing risk factors with their individual contribution values, a population exposure summary, a list of critical infrastructure within the area pulled from the Wikidata SPARQL dataset, and a count of any active ground deformation zones detected via Sentinel-1 InSAR. The result renders as a modal with a circular score indicator, contributing factors with severity-coded contribution bars, the nearest risks listed by distance with click-through navigation, and a one-click button to save the assessed location as your My Area.

#### Nearby Query

The nearby query is a focused spatial search that streams results progressively via SSE rather than waiting for the full result set. You configure a center point (manually, from the map center, or from your My Area), a radius, optional category and severity filters, and an optional source filter. The backend runs a PostGIS `ST_DWithin` query against the entire event cache and streams results in batches as they come back, sorted by distance. The results render in the same panel with severity badges, distance, category, source, and click-through navigation to either the map location or the intel bar for that event.

#### Health, Ingestion, and Cleanup Dashboards

The platform's operational state is exposed through three in-page dashboards.

The health dashboard runs live latency probes against PostGIS, every required schema table, and every external upstream API. Each probe reports OK, NOT_CONFIGURED, or FAILED with a response time, and the overall page health is aggregated into healthy/degraded/unhealthy. This is useful for both self-hosters debugging their setup and for the hosted version when a provider goes down (NOAA SWPC has done this several times during space weather events, ironically).

The ingestion dashboard shows the state of the worker that polls upstream feeds on a five-minute cadence. It surfaces whether the worker is currently running or idle, the configured interval, the current in-memory event count, totals from the most recent run, a per-category breakdown of cached events, and a list of recent ingestion runs with their status (completed, completed with errors, failed) and any error details. There is also a manual trigger button for kicking off an ingestion cycle on demand and a live SSE stream that surfaces per-source progress events as the worker fans out across categories, so you can watch the whole ingest happen in real time.

The cleanup dashboard exposes the housekeeping worker that runs every thirty minutes. The worker deletes rows with no location data, deletes events expired past their grace window, deduplicates rows with the same source/source_id pair (keeping the newest), backfills the geom column for rows that have lat/lng but no PostGIS geometry, runs ANALYZE on the events cache for query planner stats, and purges ingestion run records older than the retention window. The dashboard shows whether the worker is currently running and the full set of configuration values (batch sizes, max iterations, grace days, retention days), and has a manual trigger button.

#### Selection Mode

The selection mode is a click-to-identify feature for arbitrary geographic features that aren't in the platform's data model. Toggle it on from the header and click anywhere on the map. The Overpass API is queried for nearby polygons (buildings, parks, water bodies, natural features, administrative boundaries) using a query template that adapts to the current zoom level: at high zoom it pulls building footprints and points of interest; at medium zoom it pulls land use and natural areas; at low zoom it pulls mountain ranges, water bodies, and protected areas. The closest matching polygon by point-in-polygon test and edge distance is highlighted on the map, and the intel bar's feature mode opens with the body's name, type, area, perimeter, vertex count, and centroid, plus enriched data from reverse geocoding (Nominatim), Wikipedia (when the feature has a wikipedia tag), Open-Elevation (ground elevation), and additional Overpass queries for nearby POIs, streets, administrative boundaries, and land use context. The result is the closest thing to a free GIS identify tool you can build out of public APIs.

The Overpass integration uses multi-mirror failover across four public Overpass instances with retry and exponential backoff, plus a client-side cache with twenty-four hour TTL keyed by a hashed query string. The cache hit rate is high enough that even with the public Overpass rate limits the feature stays responsive.
